(function () {
  ...
  const goals = {
    ...
    build: ['내 문제로 Project 지침·입력·산출물을 정한다.', '내 자료로 실행해 정한 결과물을 받는다.', '내 통과 기준과 원문을 사람이 대조한다.'],
    skill: ['내 업무 절차를 SKILL.md와 ZIP으로 만든다.', 'Skill을 등록하고 다른 입력으로 다시 실행한다.', '내 검수 기준으로 확인하고 실패 규칙을 수정한다.'],
    orchestrate: ['내 직무 workflow의 입력과 출력을 정한다.', 'Orchestrator·Subagent·Skill의 역할을 구별한다.', '수정·승인·중단을 흐름에 연결한다.'],
  ...
  const checkpoints = {
    ...
    run1: ['정한 결과물을 받았다면 파일 생성 단계까지 왔습니다.', '내 성공 기준과 원문을 대조합니다. 강사 시연은 6장·10분·출처·화면을 확인하고 미검증 항목도 남깁니다.'],
    skill: ['SKILL.md와 ZIP이 생기면 파일 준비 성공입니다.', '내 Project에서 검증한 절차를 옮깁니다. 등록과 다른 입력으로 재사용하는 것은 다음 단계입니다.'],
  ...
  const diagnosis = {
    unsure: { label: '아직 모름', question: '어느 행동 직후, 무엇이 달라졌나요?', checks: ['멈춘 화면·입력 파일명·오류 문구를 먼저 기록합니다.', '계정/실행 환경 → 필수 입력 조건 → 코드·지침 순으로 좁힙니다.', '증거가 없으면 원인을 확정하지 않고 질문 하나부터 합니다.'], next: '수강생의 실수라고 단정하지 않습니다. 강사에게 마지막으로 성공한 단계와 지금 화면을 보여주세요.' },
    ...
    data: { label: '데이터 문제', question: '필수 열·ID·파일 형식이 맞나요?', checks: ['id, score, comment 열과 올바른 파일인지 확인합니다.', '빈 셀과 누락된 열을 구분하고 중복·누락 ID를 찾습니다.', '실제 자료 대신 허용된 작은 샘플로 같은 문제가 나는지 확인합니다.'], next: '없는 값은 추측해 채우지 않습니다. 필수 자료를 요청하거나 필수 입력 조건을 사용자와 다시 정합니다.' },
  ...
  function verification(slide) {
    ...
    const expected = isB ? '다른 입력에도 같은 기준을 적용합니다. 강사 시연: 인수인계 주제·S04~S05.' : '내가 정한 산출물·완료 조건을 확인합니다. 강사 시연: 6장·10분·원문 근거.';
    return '<section class="verification-strip" aria-label="결과 검증 순서"><div><span>01 / 실행 전</span><b>예상 결과를 말한다</b><p>' + expected + '</p></div><div><span>02 / 실행 후</span><b>실제 파일과 비교한다</b><p>형식·분량·필수 항목·차이를 확인합니다. 파일 생성과 내용 통과는 다릅니다.</p></div><div><span>03 / 사람 확인</span><b>원본으로 직접 대조한다</b><p>내 검수 기준으로 근거·누락·내용을 확인합니다. 확인하지 못한 부분은 미검증으로 남깁니다.</p></div></section>';
  }
