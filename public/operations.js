(function () {
  ...
  const goals = {
    ...
    build: ['Project에 지침·기준·양식을 넣는다.', 'A로 실행해 분류 CSV와 답변 초안 MD를 받는다.', '8개 ID·분류 합계·원문과 FAQ를 대조한다.'],
  ...
  function verification(slide) {
    ...
    const expected = isB ? 'B의 6개 ID·합계 6·B06 미응답, B05는 데이터로 처리.' : 'A의 8개 ID·합계 8·A08 미응답. CSV와 답변 초안 MD.';
    return '<section class="verification-strip" aria-label="결과 검증 순서"><div><span>01 / 실행 전</span><b>예상 결과를 말한다</b><p>' + expected + '</p></div><div><span>02 / 실행 후</span><b>실제 파일과 비교한다</b><p>파일명·행 수·ID·차이를 확인합니다. 파일 생성과 내용 통과는 다릅니다.</p></div><div><span>03 / 사람 확인</span><b>원본으로 직접 대조한다</b><p>전체 ID·합계를 대조하고 원문 인용과 FAQ 근거를 확인합니다. 답변 내용의 타당성은 별도로 검토합니다.</p></div></section>';
  }
