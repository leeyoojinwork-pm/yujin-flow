(function () {
  ...
  function refreshPromptPreviews() { $$('[data-lab-prompt]').forEach(x => x.textContent = E.labPrompt(x.dataset.labPrompt, state)); }
  ...
  function renderSlide(n, focus = false) {
    ...
    if (s.type === 'hero') {
      article.innerHTML = '<div class="slide-inner">' + header(s, active) + '<div class="hero-copy"><div class="hero-name">YUJIN FLOW</div><h1>내 일을,<br><span class="accent">일하는 AI로.</span></h1><div class="hero-corner" aria-hidden="true">↗</div></div><div class="hero-bottom"><p>' + esc(s.subtitle) + '</p><div class="hero-signature"><b>이유진</b>2026.09.11 · 19:30–21:30</div></div><div class="hero-strip"><span><b>Y</b>Why Gap</span><span><b>U</b>User Job</span><span><b>J</b>Journey</span><span><b>I</b>Intelligence Fit</span><span><b>N</b>Next Agent</span></div><div class="hero-start">' + button('오늘의 흐름', 'next', 'arrow-right', 'lime') + '</div></div>';
  ...
  function body(s) {
    switch (s.type) {
      ...
      ].map((x, i) => '<div class="deliverable"><div class="num">0' + (i + 1) + ' / OUTPUT</div>' + icon(x[0]) + '<h3>' + x[1] + '</h3><p>' + x[2] + '</p></div>').join('') + '</div><div class="schedule">' + D.chapters.map(c => '<div><small>' + c.n + ' / ' + c.en + '</small><b>' + c.name + '</b><span>' + c.minutes + '분</span></div>').join('') + '</div>';
      ...
      case 'framework': return '<div class="framework-grid">' + [
        ['Y', 'Why Gap', '원하는 상태와 지금 상태의 차이.', 'problem.md'],
        ['U', 'User Job', '누가 어떤 결과를 얻으려는지.', 'job-story.md'],
  ...
  function notebook() {
    ...
    $('#notebook').innerHTML='<div class="notebook-header"><div><span class="eyebrow">MY WORKSPACE / YUJIN FLOW</span><h1>내 실습노트</h1><p>완료 <b id="notebook-complete">'+p.complete+'</b> / '+D.labs.length+'개 · 질문 <b id="notebook-filled">'+p.filled+'</b> / '+p.total+'개 작성</p></div><div class="notebook-toolbar">'+button('전체 프롬프트 복사','copy-all','copy','secondary')+button('전체 MD 저장','download-all','download','primary')+'</div></div><div class="notebook-list">'+D.labs.map((l,i)=>'<details class="notebook-lab" data-notebook-lab="'+l.id+'" '+(openIds.includes(l.id)||(!openIds.length&&i===0)?'open':'')+'><summary><span class="lab-letter">'+String(i+1).padStart(2,'0')+'</span><h2>'+esc(l.title)+'</h2><span class="summary-count" data-summary-count="'+l.id+'">'+labCount(l.id)+'</span>'+icon('chevron-down')+'</summary>'+form(l.id,'notebook')+'</details>').join('')+'</div><div class="notebook-bottom"><p>내 입력은 이 브라우저의 로컬 저장소에만 남습니다.<br>브라우저 데이터 삭제 전에는 MD로 보관하세요.</p><button type="button" class="text-button" data-action="reset-all">'+icon('trash-2')+'내 기록 초기화</button></div>';
    window.YFTimers.refresh();
    icons();
  }
  ...
  function missionBrief(){return window.YFMission?window.YFMission.brief(button,icon):'';}
