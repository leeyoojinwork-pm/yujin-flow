(function () {
  ...
  const caseDesign = {
    jobseeker: {
      problem: '공고 요건과 내 경험 연결의 어려움 → 반복 수정·직무 적합성 설명 부족',
      rationale: '공고별 맞춤 작성: 요건 추출 → 경험 매칭 → 초안 작성 → 별도 사실 검수',
      review: [
        ['입력 전 · 지원자', '실제 역할·기간·성과 수치 확인'],
        ['제출 전 · 지원자', '경험·수치 원본 대조 → 설명할 수 없는 성과 삭제·수정'],
        ['근거 부족 시', '기록에 없는 “전환율 30% 개선” 삭제 → 지원자에게 질문 · 성과 창작 금지']
      ]
    },
    feedback: {
      problem: 'CS 담당자의 FAQ 반복 탐색·수작업 집계 → 첫 답변 지연·문의 누락',
      rationale: '반복 문의는 FAQ로 안내 → 남은 문의 분류·FAQ 확인·초안 작성 → 별도 누락 검수',
      review: [
        ['입력 전 · CS 담당자', 'CSV 익명화 · 분류 기준·FAQ 최신본 확인'],
        ['발송 전 · CS 담당자', 'ID·합계·FAQ 인용 대조 · 빈 문의 A08의 미응답 포함 확인'],
        ['FAQ에 없는 문의', '답변 보류 → 담당자 판단 · 환불·계정 변경·발송 자동 실행 금지']
      ]
    },
    dev: {
      problem: '불완전한 버그 제보 → 추정 기반 수정·반복 재현 → 개발자의 해결 시간 증가',
      rationale: '추정 대신 재현 근거 확보 → AI 코드 탐색·최소 수정 → 독립 리뷰·실제 테스트',
      review: [
        ['수정 전 · 개발자', '재현 조건·기대 동작·수정 범위 확정'],
        ['PR 승인 전 · 개발자', 'diff·테스트 로그 대조 · 회귀·보안 영향 확인 · 자동 병합 금지'],
        ['테스트 실패 시', '실패 로그 기반 수정 → 2회 실패·요구 충돌 시 중단·질문']
      ]
    },
    hr: {
      problem: '신입사원의 규정 탐색 어려움 → 반복 문의·HR의 버전·적용 범위 재확인',
      rationale: '체크리스트로 반복 문의 축소 → 남은 문의의 조항·적용 범위 확인 → 근거 인용 답변',
      review: [
        ['입력 전 · HR 담당자', '승인된 최신 정책·정보 공개 범위 확인'],
        ['답변 확정 전 · HR 담당자', '조항·시행일·대상자 원문 대조 → 적용 여부 판단'],
        ['개인별 예외 발생 시', '답변 확정 보류 → HR 이관 · 예외 승인·권한 부여는 담당자 결정']
      ]
    },
    planning: {
      problem: '흩어진 VOC·개별 요청의 혼재 → 기획자의 근거 부족·성급한 기능 제안',
      rationale: '수집 양식 통일 → 원문 ID별 근거 묶기 → 사실·가설 분리 → PoC 계획·PRD 초안',
      review: [
        ['분석 전 · PM', '수집 기간·사용자 범위·중복 여부 확인'],
        ['범위 승인 전 · PM', '요구사항별 VOC·지표 대조 → 관찰 사실과 미검증 가설 구분'],
        ['근거가 적을 때', 'VOC 2건의 전체 일반화 금지 → 인터뷰·PoC 설계 · 우선순위·출시는 PM 결정']
      ]
    },
    marketing: {
      problem: '마케터의 채널별 개별 제작 → 제품 정보 불일치·미검증 효능 문구 혼입',
      rationale: '정보 일관성 확보: 제품 팩트 통합 → 채널별 초안 생성 → 별도 팩트·브랜드 검수',
      review: [
        ['작성 전 · 마케터', '제품 팩트·타깃·브랜드 가이드·허용 주장 확정'],
        ['게시 전 · 마케터', '수치·효과·조건 원본 대조 · 채널별 표현·링크 확인'],
        ['검증 불가 주장 발견 시', '해당 카피 보류·수정 → 게시·광고 집행·예산 변경은 별도 승인']
      ]
    }
  };
  Object.entries(caseDesign).forEach(([key, design]) => { cases[key].design = design; });
  const guides = {
  ...
  lab('hmw', 'define');
  s('experiment', 'define', 'poc-prompt', 'I: HTML 프로토타입으로\n먼저 확인합니다.', { note: '발표 대본: 방금 만든 업무 흐름을 이제 화면으로 확인합니다. HTML 프로토타입은 예쁜 웹사이트가 아니라, Agent가 실제로 일한다면 사람이 무엇을 보고 검수해야 하는지 확인하는 화면입니다. 대시보드, 체크리스트, 검토 화면, 상태판 모두 가능합니다. 중요한 것은 디자인 완성도가 아니라 사람이 판단해야 할 정보가 화면에 보이는지입니다. CS 문의 Agent라면 문의 원문, 분류 결과, FAQ 근거, 승인 지점이 보여야 합니다. 강의자료 Agent라면 장표 목적, 흐름, 누락 자료, 수정 요청 지점이 보여야 합니다. 지금은 개발 시간이 아니라 검수 조건을 찾는 시간입니다.' });
  s('team-share-poc', 'define', 'team-task', '팀끼리 문제와 화면을\n짧게 확인합니다.', { note: '발표 대본: 여기까지 각자 문제정의, HMW 대안, 첫 번째 확인 지점까지 잡았습니다. 이제 팀 안에서 한 번만 짧게 확인합니다. 좋은 아이디어를 더 내는 시간이 아니라, 오늘 HTML 프로토타입으로 확인할 범위를 줄이는 시간입니다. 한 사람씩 30초 안에 내가 고른 문제, Agent가 봐야 할 입력, 사람이 검수할 지점을 말합니다. 듣는 사람은 빠진 조건 하나만 질문합니다. 마지막에는 “오늘은 무엇을 보여주는 화면을 만든다”라는 한 문장만 남깁니다. 시간은 6분입니다.' });
  ...
  s('workflow-map', 'map', 'workflow-example', 'J: 업무 흐름을\n예시 순서도로 봅니다.', { note: '발표 대본: 팀에서 오늘 확인할 범위를 줄였으니, 이제 그 범위를 업무 순서로 바꿔봅니다. Agent는 “알아서 잘 처리해줘”라고 하면 잘 움직이지 않습니다. 무엇을 먼저 읽고, 어떤 기준으로 분류하고, 어디서 사람이 확인하고, 마지막에 무엇을 남길지가 필요합니다. 이 페이지에서는 완벽한 시스템 설계도가 아니라 입력부터 검수까지의 주요 행동만 동사로 이어 붙입니다. 예를 들면 CS 문의라면 문의 읽기, 유형 분류하기, FAQ 찾기, 초안 쓰기, 사람 검수하기, 승인 남기기입니다. 팀에서 확인한 비어 있는 조건 하나를 이 흐름 안에 넣어봅니다.' });
  ...
  s('what-is-agent', 'map', 'concepts', '어디까지를 Agent라고 부를까요?', { source: 'effective', note: '발표 대본: 여기서 한 번 용어를 정리하고 갈게요. Prompt는 한 번의 요청입니다. “이 CSV를 분류해줘”처럼 지금 당장 시키는 한 번짜리 작업입니다. Workflow는 정해둔 단계와 분기를 따라가는 흐름입니다. 읽기, 분류, 집계, 검수처럼 사람이 미리 정한 순서가 있고, AI는 그 순서를 따라갑니다. Agent는 여기서 한 단계 더 나아가 목표와 관찰 결과를 보고 다음 행동이나 도구를 선택하는 구조입니다. 예를 들어 누락을 발견하면 다시 입력을 읽고, 기준에 안 맞으면 수정하고, 애매하면 사람에게 넘기는 식입니다. Project와 Skill은 Agent 자체라기보다 Agent가 일을 잘 하도록 돕는 환경과 재사용 절차입니다. Project는 공통 맥락과 기준 문서를 담는 공간이고, Skill은 반복해서 쓸 절차를 파일로 저장해둔 것입니다. 그래서 오늘 실습은 완전 자율 Agent를 만드는 수업이라기보다, Agent처럼 일하게 만드는 workflow와 지침을 먼저 만드는 수업입니다.' });
  ...
  s('loop-control', 'orchestrate', 'flow', 'Loop에도\n멈출 기준이 필요합니다.', { case: 'feedback', note: '검수에서 실패를 선택하면 보정 경로를 한 번 보여준다. 최대 2회 수정 후 중단. 실제 실행에서는 가드가 지침에만 머무는지 런타임에서 강제되는지도 확인한다. 직무별 상세 예시는 앞의 예시 갤러리 한 장으로 압축했다.' });
  lab('ownflow', 'ship');
