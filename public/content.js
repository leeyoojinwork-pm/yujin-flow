(function () {
  ...
  const caseDesign = {
    jobseeker: {
      problem: '취준생이 공고마다 요구하는 역량과 자신의 경험을 연결하지 못해, 지원서를 반복해서 고치고도 적합성을 설명하기 어렵다.',
      rationale: '공통 자기소개서만으로는 공고별 요건 차이를 놓친다. 요건 추출 → 경험 근거 연결 → 초안 작성으로 나누고, 사실 검수를 별도로 둔다.',
      review: [
        ['입력 전 · 지원자', '공고와 경험 기록에서 본인이 한 일·기간·성과 수치를 확인한다.'],
        ['제출 전 · 지원자', '초안의 경험·수치를 원본과 대조한다. 직접 설명할 수 없는 성과는 빼거나 고친다.'],
        ['근거 부족 시', '경험 기록에 없는 “전환율 30% 개선”은 삭제하고 질문으로 남긴다. AI가 성과를 채워 넣지 않는다.']
      ]
    },
    feedback: {
      problem: '문의가 몰릴 때 CS 담당자가 FAQ를 반복해서 찾느라 첫 답변이 늦어지고, 수작업 집계에서 일부 문의를 놓친다.',
      rationale: 'FAQ 안내만으로 해결할 문의는 먼저 줄인다. 남은 문의는 유형 분류 → FAQ 확인 → 답변 초안으로 처리하고, 누락 검사를 따로 둔다.',
      review: [
        ['입력 전 · CS 담당자', 'CSV를 익명화하고 분류 기준과 FAQ가 최신인지 확인한다.'],
        ['발송 전 · CS 담당자', '원문 ID·합계와 FAQ 인용을 대조한다. 빈 문의 A08도 미응답으로 포함됐는지 확인한다.'],
        ['FAQ에 없는 문의', '답변 생성을 보류하고 담당자가 판단한다. 환불·계정 변경·고객 발송은 자동 실행하지 않는다.']
      ]
    },
    dev: {
      problem: '개발자가 불완전한 버그 제보만으로 원인을 추정해 수정하면서, 재현과 회귀 확인을 반복하느라 해결이 늦어진다.',
      rationale: '제보 양식과 재현 테스트부터 갖춘다. AI는 코드 탐색·최소 수정안을 맡고, 독립 리뷰와 실제 테스트로 수정 근거를 확인한다.',
      review: [
        ['수정 전 · 개발자', '재현 조건과 기대 동작, 수정 가능한 범위를 확정한다.'],
        ['PR 승인 전 · 개발자', 'diff와 테스트 로그를 직접 보고 회귀·보안 영향을 확인한다. 테스트 통과만으로 병합하지 않는다.'],
        ['테스트 실패 시', '실패 로그에 맞춰 수정한다. 2회 실패하거나 요구가 충돌하면 중단하고 질문한다.']
      ]
    },
    hr: {
      problem: '신입사원이 필요한 규정을 찾지 못해 같은 질문을 반복하고, HR 담당자는 문서 버전과 개인별 적용 범위를 매번 확인한다.',
      rationale: '체크리스트로 해결할 질문은 먼저 줄인다. 나머지는 관련 조항 검색 → 시행일·적용 범위 확인 → 인용 답변으로 연결한다.',
      review: [
        ['입력 전 · HR 담당자', '승인된 최신 정책과 공개 가능한 정보인지 확인한다.'],
        ['답변 확정 전 · HR 담당자', '인용한 조항·시행일·대상자를 원문과 대조하고 해당 문의에 적용되는지 판단한다.'],
        ['개인별 예외 발생 시', '초안을 확정하지 않고 HR에 넘긴다. 예외 승인과 권한 부여는 담당자가 결정한다.']
      ]
    },
    planning: {
      problem: '기획자가 흩어진 VOC에서 반복되는 불편과 개별 요청을 구분하지 못해, 근거가 약한 기능 제안부터 작성하게 된다.',
      rationale: '수집 양식을 먼저 통일한다. 원문 ID로 의견을 묶고 사실과 원인 가설을 분리한 뒤, 대안·PoC 검증 계획을 PRD 초안에 연결한다.',
      review: [
        ['분석 전 · PM', '수집 기간·사용자 범위·중복 여부를 확인해 해석할 범위를 정한다.'],
        ['범위 승인 전 · PM', '요구사항별 VOC 원문과 지표를 대조한다. 관찰 사실과 아직 검증할 가설을 구분한다.'],
        ['근거가 적을 때', 'VOC 2건을 전체 고객 요구로 일반화하지 않는다. 인터뷰나 PoC를 설계하고 우선순위·출시는 사람이 결정한다.']
      ]
    },
    marketing: {
      problem: '마케터가 여러 채널의 카피를 따로 만들면서 제품 정보와 표현이 달라지고, 검증되지 않은 효능·성과 문구가 섞인다.',
      rationale: '제품 팩트를 한곳에 모은 뒤 채널별 초안을 만든다. 생성과 팩트·브랜드 검수를 분리해, 잘 쓴 문장인지와 사실인지를 따로 확인한다.',
      review: [
        ['작성 전 · 마케터', '제품 팩트·타깃·브랜드 가이드와 사용 가능한 주장을 확정한다.'],
        ['게시 전 · 마케터', '카피의 수치·효과·조건을 제품 자료와 대조하고 채널별 표현과 링크를 확인한다.'],
        ['검증 불가 주장 발견 시', '해당 카피를 보류하거나 수정한다. 게시·광고 집행·예산 변경은 별도 승인한다.']
      ]
    }
  };
