(function () {
  ...
  const labs = [
    { id: 'gap', letter: 'Y', title: '목표 정하기: 지금과 원하는 상태 비교', minutes: 3, purpose: '원하는 상태와 지금 상태 사이의 간격을 찾는다.', task: '아래 답변을 바탕으로 해결할 업무 문제를 한 문장으로 정리하고, 관찰 사실과 확인이 필요한 가설을 나눠줘. 기능부터 정하지 말고 결과의 차이를 설명해줘.', fields: [
    ...
    { id: 'problem', letter: 'Y', title: '문제 정의하기: 사용자와 어려움 찾기', minutes: 3, purpose: '기능 이름 없이 문제정의문을 완성한다.', task: '아래의 사용자·상황·원인·어려움·근거를 이용해 문제정의문을 작성해줘. 원인이 입증되지 않았다면 가설로 표시하고 검증 질문을 제안해줘.', fields: [
    ...
    { id: 'job', letter: 'U', title: '결과 정하기: 사용자가 해낼 일 쓰기', minutes: 2, purpose: '상황·행동·결과를 연결해 JTBD를 쓴다.', task: '아래 답변으로 JTBD 한 문장을 만들어줘. 화면 조작이나 도구 이름에 갇히지 않도록 사용자가 얻을 결과를 포함해줘.', fields: [
    ...
    { id: 'hmw', letter: 'U', title: '대안 찾기: 세 가지 해결 방향 비교', minutes: 3, purpose: 'HMW 질문과 세 가지 대안을 비교한다.', task: '아래 HMW와 대안을 비교해줘. 업무를 없애기·단순화하기·규칙 자동화하기·AI에게 맡기기를 함께 검토하고, 가장 작게 검증할 대안 하나를 골라 이유를 말해줘.', fields: [
    ...
    { id: 'experiment', letter: 'I', title: '가능성 확인하기: 작은 입력으로 AI 테스트', minutes: 4, purpose: '작은 일을 실제 Claude에서 시험한다.', task: '아래 역할·자료·작업·통과 기준으로 한 번의 가능성 실험을 수행해줘. 자료가 없으면 먼저 요청해줘. 실제 실행 전에는 성공했다고 말하지 말고, 관찰 결과는 사용자가 기록한 경우에만 평가해줘.', fields: [
    ...
    { id: 'journey', letter: 'J', title: '흐름 설계하기: 업무 단계를 순서로 연결', minutes: 4, purpose: '업무 단위마다 입력·처리·판단·출력·검수자를 연결한다.', task: '아래 현재 업무를 시작 조건 → 입력 → 처리 → 판단 → 출력 → 검수의 순서도로 만들어줘. 단계 사이에 전달되는 자료와 담당자를 표시하고, 병렬화 가능한 단계는 근거를 설명해줘.', fields: [
    ...
    { id: 'boundary', letter: 'I', title: '역할 나누기: AI 권한과 사람 승인 정하기', minutes: 3, purpose: '권한·Loop·Human Gate를 명시한다.', task: '아래 답변으로 Agent의 실행 범위를 정해줘. 통과/수정/중단을 구분하는 조건문과 최대 반복 횟수를 써줘. 명시된 승인 전에 외부 시스템에 쓰거나 발송하지 마.', fields: [
    ...
    { id: 'instructions', letter: 'N', title: '지침 만들기: Claude Project 업무 규칙 작성', minutes: 5, purpose: 'Claude Project의 Set project instructions에 넣을 지침을 작성한다.', task: '아래 답변을 실제 Claude Project instructions로 정리해줘. 역할, 목표, 입력, 절차, 규칙, 출력, 사람 확인 조건을 빠짐없이 포함해줘. 프로젝트 제목만 보고 목적을 추측하지 마.', fields: [
    ...
    { id: 'run1', letter: 'N', title: '첫 실행하기: 결과 확인하고 수정점 기록', minutes: 5, purpose: '실제 Claude 실행과 나온 파일을 확인한다.', task: '다음 실행 기록을 분석하고 지침의 어느 부분을 바꾸면 좋을지 제안해줘. 실행 기록이 비어 있으면 먼저 실행을 요청해줘. 측정하지 않은 결과를 만들지 마.', fields: [
    ...
    { id: 'skill', letter: 'N', title: 'Skill 만들기: 반복할 절차를 파일로 정리', minutes: 6, purpose: '작고 재사용 가능한 SKILL.md와 업로드 ZIP을 만든다.', task: '아래 Skill 설계를 SKILL.md로 작성해줘. name과 description을 YAML frontmatter에 넣고 본문에 입력·절차·출력·검수·중단 조건을 포함해줘. 이 파일은 자동으로 외부 도구 권한을 부여하지 않는다.', fields: [
    ...
    { id: 'run2', letter: 'N', title: '다시 실행하기: 새 자료로 Skill 검증', minutes: 4, purpose: 'Skill의 재사용성과 실제 호출 여부를 검증한다.', task: '다음 두 번째 실행 기록으로 Skill 재사용성을 평가해줘. Skill 사용 흔적과 결과 품질을 따로 평가하고 근거가 없는 성공 판정은 하지 마.', fields: [
    ...
    { id: 'ownflow', letter: 'J', title: '내 업무에 적용하기: Agent 흐름 완성', minutes: 5, purpose: '직무 예시를 내 업무로 옮긴다.', task: '아래 설계를 전체 순서도로 표현하고 각 단계의 입력·출력·담당 역할을 붙여줘. 사람이 고정한 단계와 AI가 선택할 단계, 재시도와 중단을 구분해줘.', fields: [
    ...
    { id: 'peer', letter: 'YUJIN', title: '동료 검토받기: 빠진 조건과 개선점 찾기', minutes: 3, purpose: '문제·목적·흐름·역할·실행 근거를 서로 검토한다.', task: '다음 동료 피드백을 실행 가능한 수정 한 가지로 바꿔줘. 사실 확인이 필요한 부분을 표시해줘.', fields: [
    ...
    { id: 'metrics', letter: 'Y', title: '효과 측정하기: 시간과 품질 비교', minutes: 3, purpose: '검수와 재작업을 포함해 전후를 비교한다.', task: '아래 실측값으로 순절감 시간과 주간 절감 시간을 계산해줘. 계산식은 기존 시간 - (AI 처리 + 검수 + 재작업)이고, 결과에 주간 빈도를 곱해줘. 값이 없으면 계산하지 마. 아직 목표라면 실적과 구분해줘.', fields: [
    ...
    { id: 'pitch', letter: 'N', title: '실행 계획 세우기: 결과 공유와 다음 실험', minutes: 2, purpose: '업무 문제, 역할 분담, 산출물, 다음 실험을 설명한다.', task: '아래 답변을 상사나 동료에게 설명할 3문장으로 정리해줘. 문제, 바꾼 업무와 책임, 확인된 변화와 다음 실험을 연결해줘. 실측하지 않은 효과는 목표로 표시해줘.', fields: [
