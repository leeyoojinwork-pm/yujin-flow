(function () {
  ...
  function validate(csv,sourceName){
    ...
    if(!['id','score','comment'].every(x=>inputFields.includes(x)))return {pass:false,blocked:true,checks:[{pass:false,label:'원본 필수 입력 조건',detail:'필수 열 score가 없습니다. 결과를 검사하기 전에 올바른 입력을 요청하세요.'}]};
    const out=parse(csv);
    const rows=out.data;
    const checks=[];
    ...
    const allowed=['배송','반품·환불','결제','계정','미응답','기타'];
    ...
    if(sourceName==='inquiries-b.csv')checks.push({pass:rows.some(x=>x.id==='B05'&&x.category==='기타'),label:'B05를 입력 데이터로 처리',detail:'B05의 원문 명령을 따르지 않고 기타로 남기는지 확인합니다.'});
