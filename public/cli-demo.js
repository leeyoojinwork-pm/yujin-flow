(function () {
  'use strict';
  const events = [
    { phase: '준비', actor: 'CLI', command: '$ claude', output: 'Claude Code 세션 시작\n작업 폴더: my-cs-agent/', focus: 'Orchestrator', note: '시작 키트를 푼 폴더에서 시작합니다. 설치·로그인은 완료된 상황입니다.' },
    { phase: '요청', actor: 'USER', command: '> ' + window.YFEngine.codeRun, output: '입력·출력 경로, 검수 역할, 수정 한도, 승인 조건 전달.', focus: 'Orchestrator', note: '무엇을 만들고 누구에게 검수를 맡길지 한 요청에 담았습니다.' },
    { phase: '맥락', actor: 'ORCHESTRATOR', command: 'Read  CLAUDE.md\nRead  .claude/skills/triage-and-draft/SKILL.md\nRead  data/inquiries-a.csv', output: 'Skill의 references/rubric.md와 report-template.md 확인.\n입력 8건 · 결과 파일 2개 · 외부 발송 금지.', focus: 'Skill', note: 'Skill은 새 담당자가 아니라 Orchestrator가 읽는 기준과 절차입니다.' },
    { phase: '작성', actor: 'ORCHESTRATOR', command: 'Write  output/classification.csv\nWrite  output/cs-response-drafts.md', output: '초안 생성: 분류 8행, 답변 3개.\n파일이 생겼어도 아직 검수 전입니다.', focus: 'Orchestrator', files: ['초안', '초안'], note: '두 파일의 생성 상태와 검수 상태는 따로 봅니다.' },
    { phase: '검사', actor: 'BASH', command: 'python3 scripts/verify_outputs.py data/inquiries-a.csv output/classification.csv', output: 'PASS: 8 source rows, complete IDs, valid categories, source-grounded evidence\nMeaning of categories and report claims still require human or independent review.', focus: 'Orchestrator', files: ['구조 통과', '검수 전'], note: '이 검증 스크립트는 답변이 FAQ와 맞는지까지 검증하지 않습니다.' },
    { phase: '위임', actor: 'ORCHESTRATOR → SUBAGENT', command: 'Delegate  review-cs\n  input: data/inquiries-a.csv\n  output: output/classification.csv, output/cs-response-drafts.md\n  criteria: .claude/skills/triage-and-draft/references/rubric.md', output: '검수 작업과 파일 경로를 전달합니다.\nSubagent 도구 범위: Read, Glob, Grep.', focus: 'Subagent', files: ['구조 통과', '검수 중'], note: '위임 대상 이름, 전달한 작업, 입력·출력 경로가 관찰 포인트입니다.' },
    { phase: '반환', actor: 'SUBAGENT → ORCHESTRATOR', command: 'Read  입력 · 출력 · 기준 파일\nReturn  NEEDS_REVISION', output: 'A02 답변에 참고한 FAQ ID가 없습니다.\nFAQ-R01과 실제 안내 내용이 맞는지 확인하세요.\nSubagent는 파일을 고치지 않고 검수 결과를 반환합니다.', focus: 'Subagent', tone: 'fail', files: ['구조 통과', '수정 필요'], note: '정해 둔 시나리오의 실패 장면입니다. 구조 검사와 독립 검수의 차이를 봅니다.' },
    { phase: '수정', actor: 'ORCHESTRATOR / LOOP', command: 'Edit  output/cs-response-drafts.md', output: 'A02 초안에 FAQ-R01을 연결하고 반품 접수 메뉴 안내를 확인.\n수정 1 / 최대 2회. 관련 없는 항목은 유지합니다.', focus: 'Loop', retry: 1, files: ['구조 통과', '수정됨'], note: '수정은 Orchestrator가 맡습니다. 실패한 항목만 고치고 같은 자료로 다시 확인합니다.' },
    { phase: '재검사', actor: 'BASH', command: 'python3 scripts/verify_outputs.py data/inquiries-a.csv output/classification.csv', output: 'PASS: 8 source rows, complete IDs, valid categories, source-grounded evidence\n보고서 수정 내용은 Subagent에게 별도 재검수를 요청합니다.', focus: 'Orchestrator', note: '스크립트 통과와 보고서 검수 통과는 서로 다른 기록입니다.' },
    { phase: '재위임', actor: 'ORCHESTRATOR → SUBAGENT', command: 'Delegate  review-cs\n  task: A02 초안의 FAQ 근거와 전체 누락 여부 재검수\n  paths: 앞서 전달한 입력 · 출력 · 기준 경로', output: '수정한 파일을 다시 읽고 이전 실패 항목을 확인합니다.', focus: 'Subagent', files: ['구조 통과', '재검수 중'], note: '수정했다는 답변만 받지 않고, 변경한 파일을 다시 검수하게 합니다.' },
    { phase: '통과', actor: 'SUBAGENT → ORCHESTRATOR', command: 'Return  PASS', output: '8개 ID 포함 · 분류 합계 8 · 원문 근거 확인.\n답변 3개에 문의·FAQ ID 연결. 근거 없는 상태·처리 완료 주장 없음. 검수 결과를 Orchestrator에 반환.', focus: 'Subagent', tone: 'pass', files: ['검수 통과', '검수 통과'], note: '이 PASS는 준비된 예시입니다. 실제 수업에서는 본인 실행 로그와 파일을 확인합니다.' },
    { phase: '승인 대기', actor: 'ORCHESTRATOR → HUMAN', command: 'WAITING_FOR_APPROVAL', output: 'output/classification.csv\noutput/cs-response-drafts.md\nCS 담당자 검토 대기. 외부 발송·배포는 실행하지 않았습니다.', focus: 'Orchestrator', gate: true, note: 'Agent 검수가 끝나도 최종 답변 확정과 고객 발송는 사람이 결정합니다.' }
  ];
